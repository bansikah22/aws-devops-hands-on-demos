...
```mermaid
graph TD
    subgraph SQL["Relational (SQL) - Tables"]
        direction TB
        users(Users Table)
        posts(Posts Table)
        users -- "user_id (FK)" --> posts
    end
    subgraph NoSQL["Non-Relational (NoSQL) - Documents"]
        direction TB
...
- **Use Relational** for structured data with complex relationships/transactions (e.g., banking, e-commerce)
- **Use NoSQL** for flexible, high-volume, scalable apps (e.g., social media, IoT, big data)
---
...
```mermaid
graph TD
    subgraph MongoStructure["MongoDB Structure"]
...
Example with admin credentials:
```bash
mongosh --authenticationDatabase "admin" -u "myUserAdmin" -p "Admin#123"
```
...
```javascript
// It creates automatically on first use
...
```javascript
// MongoDB creates them on first insert too
db.createCollection("users");
db.createCollection("videos");
```

#### 3. Show all collections in current database
